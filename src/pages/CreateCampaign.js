...
export default function CreateCampaign() {
  ...
  async function onSubmit(e) {
    ...
    if (targetAmount > 10000) {
      setLoading(false);
      toast.error("type amount less than $10,000");
      return;
    }
    if (images.length > 6) {
      setLoading(false);
    ...
    // const formatted_address = "193 Wickham Terrace, Spring Hill QLD 4000, Australia"
    // const geometry_location = {lat: -27.4650982, lng: 153.0240319}
    ...
    const docRef = await addDoc(collection(db, "campaigns"), formDataCopy);
    setLoading(false);
  ...
  return (
    <main className="max-w-md px-2 mx-auto">
      <h1 className="text-3xl text-center mt-6 font-bold">Create Campaign</h1>
      <h1 className="text-2xl text-center mt-6 font-normal">
      ...
      <form onSubmit={onSubmit}>
        ...
        <button
          type="submit"
          className="mb-6 w-full px-7 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 bg-origin-border text-white font-medium text-sm uppercase rounded shadow-md hover:bg-indigo-700 hover:shadow-lg focus:bg-indigo-700 focus:shadow-lg active:bg-indigo-800 active:shadow-lg transition duration-150 ease-in-out"
        >
          Publish
        </button>
      </form>
